...
try:
    from repositories.llama_cpp.llama_cpp import llama, llama_cpp
...
if TYPE_CHECKING:
    ...
    from app.utils.langchain.chat_llama_cpp import CustomLlamaCpp


cached: dict[str, "CustomLlamaCpp"] = {}
    ...
    try:
        ...
        if cache_only_single_model:
            ...
            for cached_model_name in [
                ...
                assert isinstance(client, llama.Llama)
        ...
        if model_name not in cached:
            ...
            custom_llama_cpp = get_llama(llama_cpp_model)
            assert isinstance(custom_llama_cpp.client, llama.Llama)
            if llama_cpp_model.cache and llama_cpp_model.cache_size is not None:
                custom_llama_cpp.client.set_cache(
                    llama.LlamaCache(capacity_bytes=llama_cpp_model.cache_size),  # type: ignore
                )
            cached[model_name] = custom_llama_cpp
...
def llama_cpp_generation(
    ...
    def get_generator() -> Iterator[Any]:
        ...
        if not is_fake:
            llm = load_llama(llama_cpp_model=llama_cpp_model)
            llm_client: llama.Llama = llm.client
            llm_client.verbose = echo
            real_max_tokens: int = max_tokens

            prompt_tokens: int = len(llm_client.tokenize(b" " + prompt.encode("utf-8")))
            assert llm_client.ctx is not None
            ctx_tokens: int = getattr(
                llm_client, "_n_ctx", llama_cpp.llama_n_ctx(llm_client.ctx)
            )
...
def get_llama(llama_cpp_model: "LlamaCppModel") -> "CustomLlamaCpp":
    from app.utils.langchain.chat_llama_cpp import CustomLlamaCpp
    ...
    custom_llama_cpp = CustomLlamaCpp(
        client=None,
        cache=None,
        callbacks=None,
...
class LlamaTokenizerAdapter:
    ...
    def encode(text: str, llama_cpp_model: "LlamaCppModel") -> list[int]:
        custom_llama_cpp = load_llama(llama_cpp_model=llama_cpp_model)
        assert isinstance(custom_llama_cpp.client, llama.Llama)
        return custom_llama_cpp.client.tokenize(b" " + text.encode("utf-8"))
