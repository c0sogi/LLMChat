...
class SecretConfigSetup:
    ...
    def encrypt(self) -> None:
        ...
        with open(self.json_file_name, "r") as f:
            plain_data: any = orjson.load(f)

        # Encrypt the string using the encryption key
        encrypted_data: bytes = fernet.encrypt(orjson.dumps(plain_data).encode())
...
def encode_from_utf8(text: str) -> str:
    ...
    if len(matches) == 0:
        ...
        for match in matches:
            encoded_text: bytes = b64encode(match.encode("utf-8")).decode("utf-8")
